...
fn sequence_of_two_protein_codons_translates_into_proteins() {
    let info = parse(make_pairs());
    assert_eq!(info.of_rna("UUUUUU"), Some(vec!["phenylalanine", "phenylalanine"]));
}
...
fn translate_rna_strand_into_correct_protein_list() {
    let info = parse(make_pairs());
    assert_eq!(info.of_rna("AUGUUUUGG"), Some(vec!["methionine", "phenylalanine", "tryptophan"]));
}
...
fn translation_stops_if_stop_codon_at_end_of_three_codon_sequence() {
    let info = parse(make_pairs());
    assert_eq!(info.of_rna("AUGUUUUAA"), Some(vec!["methionine", "phenylalanine"]));
}
...
fn translation_stops_if_stop_codon_in_middle_of_six_codon_sequence() {
    let info = parse(make_pairs());
    assert_eq!(info.of_rna("UGGUGUUAUUAAUGGUUU"), Some(vec!["tryptophan", "cysteine", "tyrosine"]));
}
...
fn incomplete_rna_sequence_can_translate_if_valid_until_a_stop_codon() {
    let info = parse(make_pairs());
    assert_eq!(info.of_rna("UUCUUCUAAUGGU"), Some(vec!["phenylalanine", "phenylalanine"]));
}
